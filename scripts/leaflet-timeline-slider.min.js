(L.Control.TimeLineSlider = L.Control.extend({
  ...
  initialize(options) {
    typeof options.changeMap !== "function" &&
  ...
  onAdd(map) {
    ...
    L.DomEvent.on(this.container, "control_container", (ev) => {
    ...
    (this.rangeLabels.innerHTML = this.options.timelineItems.map((item) => `<li>${item}</li>`).join("")),
    ...
    (this.rangeWidthCSS =
      parseFloat(this.options.labelWidth) * (this.options.timelineItems.length - 1) + 2 * this.thumbSize),
    (this.rlLabelMargin = parseFloat(this.options.labelWidth) / 2 - parseFloat(this.options.thumbHeight) / 2),
    (this.topLabelMargin =
      parseFloat(this.options.betweenLabelAndRangeSpace) - parseFloat(this.options.thumbHeight) - 2.5),
    ...
    L.DomEvent.on(this.rangeInput, "input", function () {
      ...
      const curLabel = that.rangeLabelArray[curValue - 1].innerHTML;
      (mapParams = { value: curValue, label: curLabel, map }),
    ...
    this.rangeLabelArray))
      L.DomEvent.on(li, "click", (e) => {
        ...
        const index = that.rangeLabelArray.indexOf(targetli);
  ...
  hexToRGBA(hex, opacity) {
    ...
    if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex))
      return (
        (c = hex.substring(1).split("")).length == 3 && (c = [c[0], c[0], c[1], c[1], c[2], c[2]]),
        `rgba(${[((c = `0x${c.join("")}`) >> 16) & 255, (c >> 8) & 255, 255 & c].join(",")},${opacity})`
      );
  ...
  setupStartStyles() {
    return (
      (style = `\n            .control_container { \n                background-color: ${
        that.backgroundRGBA
      };\n                padding: ${that.options.topBgPadding} ${that.options.rightBgPadding} ${
  ...
  getTrackStyle(el, sliderLength) {
    ...
    const labelIndex = curVal - 1;
    const val = labelIndex * (100 / (sliderLength - 1));
    const coverVal = (parseFloat(that.thumbSize) / that.rangeWidthCSS) * 100;
    for (li of ((style = ""), that.rangeLabelArray))
      L.DomUtil.removeClass(li, "active"), L.DomUtil.removeClass(li, "selected");
    const curLabel = that.rangeLabelArray[labelIndex];
    for (L.DomUtil.addClass(curLabel, "active"), L.DomUtil.addClass(curLabel, "selected"), i = 0; i < curVal; i++)
    ...
    for (var i = 0; i < prefs.length; i++)
      (style += `.range {background: linear-gradient(to right, ${that.coverBackgroundRGBA} 0%, ${that.coverBackgroundRGBA} ${coverVal}%, ${that.options.activeColor} ${coverVal}%, ${that.options.activeColor} ${val}%,  ${that.coverBackgroundRGBA} 0%, ${that.coverBackgroundRGBA} 100%)}`),
        (style += `.range input::-${prefs[i]}{background: linear-gradient(to right, ${that.coverBackgroundRGBA} 0%, ${
          that.coverBackgroundRGBA
        } ${coverVal}%, ${that.options.activeColor} 0%, ${that.options.activeColor} ${val}%, ${
          that.options.inactiveColor
        } ${val}%, ${that.options.inactiveColor} ${100 - coverVal}%, ${that.coverBackgroundRGBA} ${100 - coverVal}%, ${
          that.coverBackgroundRGBA
        } 100%)}`);
    return style;
  },
