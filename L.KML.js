...
L.Util.extend(L.KML, {
  ...
  parseStyle(xml, kmlOptions) {
    let style = {}; let poptions = {}; let ioptions = {}; let el; let
      id;
    ...
    function _parse(xml) {
      ...
      for (let i = 0; i < xml.childNodes.length; i++) {
        ...
        if (key === 'hotSpot') {
          ...
          const value = (e.childNodes && e.childNodes.length) ? e.childNodes[0].nodeValue : null;
          if (!value) {
            continue;
          }
          if (key === 'color') {
  ...
  parsePlacemark(place, xml, style, options) {
    let h; let i; let j; let k; let el; let il; const
      opts = options || {};
  ...
  parsePolygon(line, xml, options) {
    let el; const polys = []; const inner = []; let i; let
      coords;
